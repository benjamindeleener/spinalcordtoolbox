...
% Applying tranform to the GM tract files and copying geometry and saving
for label = 1:length(label_GM_left)
    label_GM_g = label + length(label_left) + length(label_right)
    label_GM_d = label + length(label_left) + length(label_right) +length(label_GM_left)
    tract_atlas_GM_g = [path_out 'tract_atlas_' num2str(label_GM_g)];
    tract_atlas_GM_d = [path_out 'tract_atlas_' num2str(label_GM_d)];

    
    cmd = ['isct_antsApplyTransforms -d 2 -i ' tract_atlas_GM_g ext ' -o ' tract_atlas_GM_g suffix_ants ext ' -t ' Warp_atlas ' ' path_out affine_atlas ' -r ' templateci_slice_ref_thresh ext];
    disp(cmd); [status,result] = unix(cmd); if(status), error(result); end, disp(result)
    
    cmd = ['isct_antsApplyTransforms -d 2 -i ' tract_atlas_GM_d ext ' -o ' tract_atlas_GM_d suffix_ants ext ' -t ' Warp_atlas ' ' path_out affine_atlas ' -r ' templateci_slice_ref_thresh ext];
    disp(cmd); [status,result] = unix(cmd); if(status), error(result); end, disp(result)
    
    tract_reg_GM_g = [path_out 'tract_atlas_' num2str(label_GM_g) suffix_ants];
    temp_GM_g = read_avw(tract_reg_GM_g);
    
    tract_reg_GM_d = [path_out 'tract_atlas_' num2str(label_GM_d) suffix_ants];
    temp_GM_d = read_avw(tract_reg_GM_d);
    
    % Replace isolated values with the mean of the adjacent values
    for i = 2:size(temp_GM_g,1)-1
        for j = 2:size(temp_GM_g,2)-1
            test = (temp_GM_g(i,j)==temp_GM_g(i-1,j)) || (temp_GM_g(i,j)==temp_GM_g(i,j-1)) || (temp_GM_g(i,j)==temp_GM_g(i+1,j)) || (temp_GM_g(i,j)==temp_GM_g(i,j+1));
            if(~test)
                temp_GM_g(i,j) = (temp_GM_g(i-1,j) + temp_GM_g(i+1,j) + temp_GM_g(i,j+1) + temp_GM_g(i,j-1))/4;
            end
        end
    end
    
    for i = 2:size(temp_GM_d,1)-1
        for j = 2:size(temp_GM_d,2)-1
            test = (temp_GM_d(i,j)==temp_GM_d(i-1,j)) || (temp_GM_d(i,j)==temp_GM_d(i,j-1)) || (temp_GM_d(i,j)==temp_GM_d(i+1,j)) || (temp_GM_d(i,j)==temp_GM_d(i,j+1));
            if(~test)
                temp_GM_d(i,j) = (temp_GM_d(i-1,j) + temp_GM_d(i+1,j) + temp_GM_d(i,j+1) + temp_GM_d(i,j-1))/4;
            end
        end
    end
    
    % Symmetry constraint for left and right tracts
    temp_GM_sum = temp_GM_g + temp_GM_d;
    temp_GM_sum_flip = temp_GM_sum(end:-1:1,:);
    temp_GM_sym = (temp_GM_sum + temp_GM_sum_flip) / 2;
    
    temp_GM_g(1:end/2,:) = 0;
    temp_GM_g(1+end/2:end,:) = temp_GM_sym(1+end/2:end,:);
    temp_GM_d(1:end/2,:) = temp_GM_sym(1:end/2,:);
    temp_GM_d(1+end/2:end,:) = 0;
    
    tractsHR{label_GM_g}(:,:,num_slice_ref) = temp_GM_g;
    tractsHR{label_GM_d}(:,:,num_slice_ref) = temp_GM_d;
end

% Apply tranform to the PVE tract files
for label = length([label_left, label_right, label_GM_left, label_GM_right])+1:length(label_values)
    tract_atlas = [path_out 'tract_atlas_' num2str(label)];
...
%     cmd = ['sct_WarpImageMultiTransform 2 ' atlas_nifti ext ' ' atlas_slice ext ' ' warp_slice ' ' Warp_atlas ' ' path_out affine_atlas ' -R ' templatecit_slice ext];
    ...
    for label = 1:length(label_GM_left)
        label_l = length(label_left) + length(label_right) + label;
        label_r = length(label_left) + length(label_right) +length(label_GM_left) + label;
        tract_atlas_GM_g = [path_out 'tract_atlas_' num2str(label_l)];
        tract_atlas_GM_d = [path_out 'tract_atlas_' num2str(label_r)];

        % LEFT
        cmd = ['isct_antsApplyTransforms -d 2 -i ' tract_atlas_GM_g ext ' -o ' tract_atlas_GM_g suffix_ants ext ' -t ' Warp_atlas ' ' path_out affine_atlas ' -r ' templateci_slice_ref_thresh ext];
        disp(cmd); [status,result] = unix(cmd); if(status), error(result); end, %disp(result)
        cmd = ['isct_c3d ' templatecit_slice_ref ' ' tract_atlas_GM_g suffix_ants ext ' -copy-transform -o ' tract_atlas_GM_g suffix_ants ext ext];  % copy geom-- added: 2014-08-30
        disp(cmd); [status,result]=unix(cmd); if(status), error(result); end, %disp(result)
        cmd = ['isct_antsApplyTransforms -d 2 -i ' tract_atlas_GM_g suffix_ants ext ' -o ' tract_atlas_GM_g suffix_ants ext ' -t ' warp_slice ' -r ' templatecit_slice ext];
        disp(cmd); [status,result] = unix(cmd); if(status), error(result); end, %disp(result)
        
        % RIGHT
        cmd = ['isct_antsApplyTransforms -d 2 -i ' tract_atlas_GM_d ext ' -o ' tract_atlas_GM_d suffix_ants ext ' -t ' Warp_atlas ' ' path_out affine_atlas ' -r ' templateci_slice_ref_thresh ext];
        disp(cmd); [status,result] = unix(cmd); if(status), error(result); end, %disp(result)
        cmd = ['isct_c3d ' templatecit_slice_ref ' ' tract_atlas_GM_d suffix_ants ext ' -copy-transform -o ' tract_atlas_GM_d suffix_ants ext ext];  % copy geom-- added: 2014-08-30
        disp(cmd); [status,result]=unix(cmd); if(status), error(result); end, %disp(result)
        cmd = ['isct_antsApplyTransforms -d 2 -i ' tract_atlas_GM_d suffix_ants ext ' -o ' tract_atlas_GM_d suffix_ants ext ' -t ' warp_slice ' -r ' templatecit_slice ext];
        disp(cmd); [status,result] = unix(cmd); if(status), error(result); end, %disp(result)

        % copy header from template to registered atlas
        % NB: changed templateci_slice to templatecit_slice (2014-08-04)
        cmd = ['isct_c3d ' templatecit_slice ext ' ' tract_atlas_GM_g suffix_ants ext ' -copy-transform -o ' tract_atlas_GM_g suffix_ants ext];
        disp(cmd); [status,result] = unix(cmd); if(status), error(result); end, %disp(result)
        
        cmd = ['isct_c3d ' templatecit_slice ext ' ' tract_atlas_GM_d suffix_ants ext ' -copy-transform -o ' tract_atlas_GM_d suffix_ants ext];
        disp(cmd); [status,result] = unix(cmd); if(status), error(result); end, %disp(result)
        
        tract_GM_reg_g = [path_out 'tract_atlas_' num2str(label_l) suffix_ants];
        temp_GM_g = read_avw(tract_GM_reg_g);
        
        tract_GM_reg_d = [path_out 'tract_atlas_' num2str(label_r) suffix_ants];
        temp_GM_d = read_avw(tract_GM_reg_d);
        ...
        % Symmetry constraint for left and right tracts
        temp_sum = temp_GM_g + temp_GM_d;
        temp_sum_flip = temp_sum(end:-1:1,:);
        temp_sym = (temp_sum + temp_sum_flip) / 2;
        
        temp_GM_g(1:end/2,:) = 0;
        temp_GM_g(1+end/2:end,:) = temp_sym(1+end/2:end,:);
        temp_GM_d(1:end/2,:) = temp_sym(1:end/2,:);
        temp_GM_d(1+end/2:end,:) = 0;
        
        tractsHR{label_l}(:,:,numSlice) = temp_GM_g;
        tractsHR{label_r}(:,:,numSlice) = temp_GM_d;
...
disp(['fslview ',path_template,'MNI-Poly-AMU_T2.nii.gz -b 0,5000 ',path_template,'MNI-Poly-AMU_WM.nii.gz', bricon, prefix_out, '00', ext, ' -l Red',bricon, prefix_out, '01', ext, ' -l Green',bricon, prefix_out, '02', ext, ' -l Blue',bricon, prefix_out, '03', ext, ' -l Yellow',bricon, prefix_out, '04', ext, ' -l Pink ',bricon, prefix_out, '30', ext, ' -l Cool ',bricon, prefix_out, '31', ext, ' -l Copper ',bricon,' &'])
